...
void main() {
  test('send/iq before connect error with StateError', () async {
    final client = XmppClient(host: 'ex', domain: 'ex', username: 'alice', password: 'secret', tls: TlsMode.none);
    await expectLater(client.send(xml('presence')), throwsA(isA<StateError>()));
    await expectLater(client.iq(xml('iq', attrs: {'type': 'get'})), throwsA(isA<StateError>()));
  });
  ...
  test('handler registered after connect answers inbound IQ', () async {
    ...
    client.onIqGet('urn:example', 'q', (iq, child) => xml('q', attrs: {'xmlns': 'urn:example'}, text: 'pong'));

    t.deliver(
      "<iq type='get' from='srv' id='e1'>"
      "<q xmlns='urn:example'/></iq>",
    );
    await pump();
    expect(t.writes.any((w) => w.contains('type="result"') && w.contains('id="e1"')), isTrue);
